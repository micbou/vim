...
# Invoke-Expression "& '$vc_vars_script_path' x86 '&' set" |
# foreach {
#     if ($_ -match "=") {
#         $key, $value = $_.Split("=")
...
Copy-Item C:\gettext\libintl-8.dll "$env:APPVEYOR_BUILD_FOLDER\runtime"
Copy-Item C:\gettext\libiconv-2.dll "$env:APPVEYOR_BUILD_FOLDER\runtime"
# Copy libwinpthread only for 64-bit.
if ($env:arch -eq 64) {
    Copy-Item C:\gettext\libwinpthread-1.dll "$env:APPVEYOR_BUILD_FOLDER\runtime"
}
